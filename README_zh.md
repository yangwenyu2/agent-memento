...
> 1. **破坏性文件清理**: 核心引擎 (`memento_tick.sh`) 运行失败时会强制执行 `git clean -fd` 与 `git checkout -- .`，这**会永久删除**项目目录内所有未被 `git` 追踪的文件（除非加入 ` .memento_cleanignore`）。**绝对不要在已有重要文件或秘密的目录中初始化本框架**。
> 2. **HTTP 静态暴露**: 启动的监控台 Dashboard 会将你指定的项目目录完全挂载为静态网站 (`/preview`) 以供全网预览。请确保您的项目内**不包含任何 API Keys、密码或私密敏感文件**，以免通过端口泄露。
> 3. **底层自动化风险**: Agent 会在后台以宿主机权限静默调用命令与网络请求（如 NPM 依赖安装）。



> ⚠️ **安全与风险声明 (Security & Risk Warning)**:
> 本技能包含高权限的自动化工程能力，请仔细阅读：
> 1. **破坏性文件清理**: 核心引擎 (`memento_tick.sh`) 运行失败时会执行 `git clean -fd` 与 `git checkout -- .`，这**会永久删除**项目目录内所有未被 `git` 追踪或未加入 ` .memento_cleanignore` 的文件。**绝对不要在已有重要文件的目录中初始化本框架**。
> 2. **HTTP 静态暴露**: 启动的监控台 Dashboard 会在您指定的端口（默认 3777）启动 HTTP 服务，并将你指定的项目目录完全挂载为静态网站 (`/preview`) 以供实时预览。请确保您的项目内**不包含任何 API Keys、密码或私密敏感文件**，以免在公网或局域网意外泄露。
> 3. **底层指令自动化**: 该 Agent 每 5 分钟在后台隐式执行（或通过循环触发），它会自动下达终端命令并修改文件系统。
